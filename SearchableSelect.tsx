...
interface SearchableSelectProps {
  ...
  className?: string;
  isLoading?: boolean;
  descriptions?: Record<string, string>; // New prop for showing details like Seat Areas
}
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (wrapperRef.current && !wrapperRef.current.contains(event.target as Node)) {
        setIsOpen(false);
        // Reset search term to currently selected value if user didn't select a new one
        if (value) {
          setSearchTerm(value);
        } else {
          setSearchTerm('');
