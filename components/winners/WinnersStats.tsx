...
interface WinnersStatsProps {
  ...
  activePartyFilter: string;
  activeStatusFilter: 'all' | 'suspended' | 'pending';
  onCardClick: (type: 'party' | 'suspended' | 'pending', value?: string) => void;
}

const WinnersStats: React.FC<WinnersStatsProps> = ({ 
    stats = {}, 
    generalStats = { suspendedCount: 0, pendingCount: 0 }, 
    leadingStats = {}, 
    showLeading, setShowLeading,
    activePartyFilter, activeStatusFilter, onCardClick
}) => {
  
  const priorityParties = ['বিএনপি', 'জামায়াতে ইসলামী', 'এনসিপি', 'স্বতন্ত্র'];
  
  const winningParties = Object.entries(stats || {})
  ...
  return (
    <div className="space-y-4">
        ...
        {winningParties.map(([party, count]) => {
            ...
            const isActive = activePartyFilter === party;
        ...
        <div 
            ...
            className={`bg-red-600 rounded-lg p-2 text-white shadow-sm flex flex-col items-center justify-center relative overflow-hidden min-h-[65px] hover:scale-105 transition-all duration-200 cursor-pointer
                ${activeStatusFilter === 'suspended' ? 'ring-2 ring-offset-2 ring-red-600 scale-105' : 'hover:shadow-md'}
            `}
        ...
        >
            ...
            <div className="text-xl font-bold leading-none">{generalStats.suspendedCount.toLocaleString('bn-BD')}</div>
            <div className="text-[9px] opacity-80">টি আসন</div>
        </div>
        ...
        <div 
            ...
            className={`bg-red-700 rounded-lg p-2 text-white shadow-sm flex flex-col items-center justify-center relative overflow-hidden cursor-pointer hover:bg-red-800 transition-all active:scale-95 border border-red-500 hover:border-red-400 min-h-[65px]
                ${activeStatusFilter === 'pending' ? 'ring-2 ring-offset-2 ring-red-700 scale-105' : 'hover:shadow-md'}
            `}
        ...
        >
            ...
            <div className="text-xl font-bold leading-none">{generalStats.pendingCount.toLocaleString('bn-BD')}</div>
            <div className="text-[9px] opacity-80">ফলাফল</div>
        </div>
        </div>

        {/* Leading Stats Section (Visible only when 'Remaining' is clicked via the small toggle, or implied if user wants to see leading) */}
        {showLeading && (
